...
function LoadItems (_jsonPath) {
    ...
    $.getJSON(_jsonPath, function(data){
        data.forEach(planoSingle => {
            appendPlan(itemsContainer, planoSingle);
        });
    }).fail(function(){
        console.log("Erro ao ler arquivo .json de 'planos'")
        appendPlan(itemsContainer, "Erro. Recarregue a página.");
    });
}
function appendPlan(_container, _planoSingle){
